...
PROGRAM_DIR = os.path.join(os.environ["ProgramFiles"], "ViewsferAgent")
TAC_RMM = os.path.join(PROGRAM_DIR, "viewsfer.exe")
...
def get_services():
    # see https://github.com/wh1te909/viewsfer/issues/38
...
def update_salt():
    for p in psutil.process_iter():
        with p.oneshot():
            if p.name() == "viewsfer.exe" and "updatesalt" in p.cmdline():
                return "running"
...
def install_updates():
    for p in psutil.process_iter():
        with p.oneshot():
            if p.name() == "viewsfer.exe" and "winupdater" in p.cmdline():
                return "running"
...
def agent_update_v2(inno, url):
    ...
    tac = _wait_for_service(svc="viewsferagent", status="running")
    if tac != "running":
        subprocess.run([NSSM, "start", "viewsferagent"], timeout=30)
...
def agent_update(version, url):
    ...
    services = ("viewsferagent", "checkrunner")
